...
#List files containing the data to create figure 1
figure1_files <- list.files(path = "C:/Users/Micah/Dropbox/2022 EDF carbon market/applied theory paper/app.theory.sim/data/figure1/",
                            pattern = '8_22*',
                            full.names = TRUE)
output_df <- lapply(figure1_files, readRDS)
...
plot_df <- output_df %>% dplyr::group_by(e_nr_sd, e_c0_sd) %>% summarise(n = n(),
                                                                         ...
                                                                         #Average product
                                                                         act_ap = mean(total_add_outc_act/total_payments_rec_act),
                                                                         res_ap = mean(total_add_outc_res/total_payments_rec_res),
...
#Additional participation - percent of all participants.
add_part_plot <- ggplot(data = data_label, aes(x = e_nr_sd, y = addpart, color = e_c_sd_factor)) +
  geom_line(aes(linetype = program), alpha = 0.75) +
  # geom_errorbar(aes(ymin = meanaddseqnt - 2*seaddseqnt, ymax = meanaddseqnt + 2*seaddseqnt),  position = dodge) +
  scale_x_continuous(limits = c(0, 1.1), breaks = c(0, 0.5, 1.0)) +
  geom_label_repel(aes(label = label),
                   ...
                   segment.alpha = 0,
                   size = 4) +
  ggtitle("Additionality of participants") +
  xlab(expression('Net return error st. dev., '*sigma[italic(epsilon)]^r)) +
  ylab("Fraction of /n total participants") +
...
ggdraw(add_part_plot)

#Additional outcome per capita.
add_outc_plot <- ggplot(data = data_label, aes(x = e_nr_sd, y = addoutcomes/10000, color = e_c_sd_factor)) +
  geom_line(aes(linetype = program), alpha = 0.75) +
  # geom_errorbar(aes(ymin = meanaddseqnt - 2*seaddseqnt, ymax = meanaddseqnt + 2*seaddseqnt),  position = dodge) +
  scale_x_continuous(limits = c(0, 1.1), breaks = c(0, 0.5, 1.0)) +
  geom_label_repel(aes(label = label),
                   ...
                   segment.alpha = 0,
                   size = 4) +
  ggtitle("Additional outcomes") +
  xlab(expression('Net return error st. dev., '*sigma[italic(epsilon)]^r)) +
  ylab("Additional ES /n per capita") +
  scale_linetype_manual(values = ltypes, labels = c('Action-based', 'Modeled-results')) +
  scale_color_manual(values = cols, guide = 'none') +
  theme(text=element_text(size=16,  family="serif"),
        plot.title = element_text(hjust = 0.5)) +
  ...
  theme(legend.title=element_blank(),
        ...
        legend.key.size = unit(1.5, "cm"),
        legend.text=element_text(size=16, hjust=0))
ggdraw(add_outc_plot)

#Average product per unit of expenditure
avg_prod_plot <- ggplot(data = data_label, aes(x = e_nr_sd, y = ap, color = e_c_sd_factor)) +
  geom_line(aes(linetype = program), alpha = 0.75) +
  # geom_errorbar(aes(ymin = meanavgcostnt - 2*seavgcostnt, ymax = meanavgcostnt + 2*seavgcostnt),  position = dodge) +
  scale_x_continuous(limits = c(0, 1.1), breaks = c(0, 0.5, 1.0)) +
  geom_label_repel(aes(label = label),
                   direction = 'y',
                   nudge_x = 0.1,
                   ...
                   segment.alpha = 0,
                   size = 4) +
  ggtitle("Program efficiency") +
  xlab(expression('Net return error st. dev., '*sigma[italic(epsilon)]^r)) +
  ylab("Additional ES / /n program cost") +  scale_linetype_manual(values = ltypes, labels = c('Action-based', 'Modeled-results')) +
  scale_color_manual(values = cols, guide = 'none') +
  theme(text=element_text(size=16,  family="serif"),
        plot.title = element_text(hjust = 0.5)) +
  theme(panel.background = element_blank()) +
  ...
  theme(legend.title=element_blank(),
        ...
        legend.key.size = unit(1.5, "cm"),
        legend.text=element_text(size=16, hjust=0))
ggdraw(avg_prod_plot)

#Grab the legend from one graph
leg_plot <- add_outc_plot + theme(legend.position = 'bottom',
                                  legend.key=element_blank(),
...
prow_fig1 <- plot_grid(
  add_part_plot + theme(legend.position="none"),
  add_outc_plot + theme(legend.position="none"),
  avg_prod_plot + theme(legend.position="none"),
  align = 'vh',
  labels = c("(a)", "(b)", "(c)"),
  label_size = 18,
...
final_fig1_plot <- plot_grid(
  prow_fig1,
  legend,
  nrow = 2,
  rel_heights = c(1, 0.1))
ggdraw(final_fig1_plot)

#Save
ggsave2(filename = "figure1_3plots_10_19.tiff",
        plot = final_fig1_plot,
        device = "tiff",
