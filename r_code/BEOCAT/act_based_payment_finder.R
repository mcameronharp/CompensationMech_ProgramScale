#Load in the program that determines the critical values given a jurisdiction's aggregate, biased baseline
#source("C:/Users/Micah/Dropbox/2022 EDF carbon market/r scripts/temp/rhat critical value finder.R")

#Define function running model

#------------------------------------------------------------------------------#
ppp_payment_finder <- function(ppp_payment,
                               input_df) {
  #Run this function after the initializing step
  working_df <- input_df
  ...
  working_df <- working_df %>% dplyr::mutate( #First create variables showing all producers net returns with the ppp payment, regardless of enrollment eligibility
  ...
  working_df <- working_df %>% dplyr::group_by(jurs_id) %>% summarise(#Now create variables showing the total number of producers who want to enroll within each jurisdiction, regardless of their individual baselines
    ...
    dplyr::left_join(working_df, by = "jurs_id")

  # PPP Jurisdictional Outcomes #
  working_df <- working_df %>% dplyr::mutate(#Create indicator showing if jurisdiction opts in - They will opt in if the return to enrollment (number enrolled above baseline times ppp payment) is
  ...
  working_df <- working_df %>% dplyr::mutate( #First create variables showing all producers net returns with the ppco2 payment, regardless of enrollment eligibility
  ...
  working_df <- working_df %>% dplyr::group_by(jurs_id) %>% summarise(#Aggregate carbon outcome if ppco2 program is available, as well as the amount that is spurious
  ...
  working_df <- working_df %>% dplyr::mutate(#Create indicator showing if jurisdiction opts in for cc and nt
  ...
  jur_output_df <- working_df %>% dplyr::select(starts_with("jur")) %>% distinct()
  ...
  #Check difference in total cost between action and results based program
  diff_cost <- total_payments_rec_res - total_payments_rec_act
}
#----------------------------END FUNCTION -------------------------------------#
